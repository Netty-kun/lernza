...
impl QuestContract {
    ...
    /// Returns a quest's completion percentage, or `None` when the quest has
    /// not started and no percentage is known. Dashboard clients should use
    /// this value directly and render an empty/"Not started" state instead of
    /// defaulting a missing value to 0%.
    pub fn get_completion_percentage(env: Env, quest_id: u32) -> Result<Option<u32>, Error> {
        Self::load_quest(&env, quest_id)?;
        Self::bump(&env, quest_id);
        Ok(None)
    }
